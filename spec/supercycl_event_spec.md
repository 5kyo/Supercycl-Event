...
> 📌 **Mock 식별자**: 본 명세에 첨부된 프로토타입(`src/lib/mock-state/initial.ts`)은 로컬 미리보기를 위해 `MOCK_OKX_UID = '1234567890'` 과 `MOCK_ACCOUNT_ADDRESS = 'hxfedcba9876…ba9876'` 을 하드코드합니다. 본 서비스 연동 시점에 OAuth 콜백/계정 프로비저닝에서 받은 실제 값으로 자연스럽게 교체되도록 컴포넌트는 이 두 상수에 직접 의존하지 않고 `state.okxUid` / `state.accountAddress`로만 접근합니다.
...
> 문항 원본은 `src/content/survey-ko.md` (KO) / `src/content/survey-en.md` (EN) 이며, 본 표는 운영 정책 참조용입니다 (실제 문항 텍스트는 마크다운 원본을 단일 source of truth로 사용).
> SurveyModal 헤더에 `KO | EN` 토글이 제공됩니다 — KO 기본, 토글 시 진행 중 응답은 초기화됩니다 (옵션 라벨이 언어별로 달라 호환되지 않기 때문). 상태는 모달 내부에서만 유지되며 모달을 닫으면 사라집니다.

| # | 영역 | 문항 | 형식 |
|---|------|------|------|
| 1 | 만족 | Supercycl을 쓰면서 가장 좋았던 점 | 다중 선택 |
| 2 | UX | 가장 조작이 어려웠던 화면/기능 | 다중 (기타 선택 시 자유 입력) |
| 3 | UX | 추가되었으면 하는 기능 | 자유 (필수) |
| 4 | 자동매매 | AI 자동매매 출시 시 사용 의향 | 5점 척도 |
| 5 | 거래소 | 현재 쓰고 있는 거래소 | 다중 (Bybit/OKX/Bitget/Binance/BingX/Gate/Hyperliquid/기타) |
| 6 ⭐ | 2x 레버리지 | 다른 거래소 평균 사용 레버리지 | 단일 (2x/3x/5x/10x+/직접 입력) |
| 7 ⭐ | 2x 레버리지 | Supercycl의 2x 레버리지 제한이 리스크 관리에 도움이 되나요 | 5점 척도 (1: 아니오 ─ 5: 예) |
| 8 | 인지 | Supercycl을 어떻게 알게 되었나요 | 단일 |
| 9 | 프로필 | 월평균 크립토 거래 금액대 | 단일 (구간) |
| 10 | 시그널 개인화 | 관심 자산 카테고리 | 다중 (BTC/ETH/Alt/Meme/RWA) |
| 11 | 피드백 | Supercycl에서 개선되었으면 하는 점이 있다면 알려주세요 | 자유 (선택) |

> 📌 **변경 이력**:
> - **이전 13문항 → 12문항** (커밋 `48609f2`): Q1 재설계, 신뢰 영역 단독 문항 제거, 번호 시프트
> - **12문항 → 10문항**: 가치제안 단일 문항(Supercycl을 쓴다면 가장 매력적인 기능 — 시그널 품질/차트/거래소 통합/수수료/리워드) 제거. 가치제안 신호는 Q1(만족) 다중 선택지로 흡수
> - **10문항 → 10문항**: 2x 레버리지 입장 문항(`Supercycl의 2x 제한에 대한 본인의 입장`) 제거. 이미 Q7(5점 척도)에서 2x 제한이 리스크 관리에 도움이 되는지 측정하고 있어 동일 영역의 다중 선택 문항은 중복.
> - **10문항 → 11문항**: 마지막에 자유 입력 피드백 문항(`개선되었으면 하는 점`) 추가 (선택 입력). 정성 피드백을 한 줄 모아 마무리 신호로 사용.
> - 문항 카운트는 `surveyKo.length` 로부터 자동 계산되어 SurveyModal 진행률·SurveyCompleteModal "N / N complete" 표시에 반영됨. 스펙은 문서 정합성 차원에서 동기화 필요.
...
**전환 압력 3축:** 진행률 + 종료까지 시간 + (USDT 카드 내) 잔여 슬롯. 슬롯 라인은 진척도 영역에서 제거하고 USDT 보상 카드에 단일 소스로 통합(§5.3).
...
| 안내 표시 | USDT 보상 카드: 상단 `Closed` chip + 금액 strikethrough + 조건 라인 (`Trade window closed — reward no longer available`) + 카드 내 슬롯 sub-block이 `full` 단계(0 / 500 + **FULL** 배지)로 전환. Survey 트랙 및 ICX 보상은 그대로 진행 (Hub 본문 유지) |
...
| **6/29 ~ 7/5** | 설문 완료 (ICX 지급) | 이벤트 페이지에서 11문항 응답 → 즉시 미니 리포트 (수령 정보 입력 단계 없음 — 연동된 OKX UID로 자동 송금) | 외부 이벤트 페이지 |
